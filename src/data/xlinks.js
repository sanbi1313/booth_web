// 부스 페이지에서 프로필 사진 클릭 시 이동할 X(트위터) 링크를 관리합니다.
// artists.js의 key 값과 동일한 key를 사용하세요. 값이 빈 문자열이면 클릭해도 이동하지 않습니다.
export const xLinks = {
  iv: 'https://x.com/ccspd123',
  okkimon: 'https://x.com/_okkizzz',
  jw: 'https://x.com/Wrdajw0',
  nyang: 'https://x.com/Naring22',
  natsu: 'https://x.com/acidrain1313',
  aseo: 'https://x.com/arthur_0323',
  weedrug: 'https://x.com/aworuw1137',
  roroko: 'https://x.com/loloco_0920',
}
